...
#Importación   --------------------------------------------------------
data <- read_sheet("12Nt_zqekkR7PBDyf5eQ3V8mHnQD65qXs6YVBBDWo7rQ") %>% 
  select(1:4,genero,generacion,tiempo_laboral) %>% 
  mutate(`Submitted at` = as.POSIXct(`Submitted at`),
         fecha = `Submitted at`- hours(6))

principal <- read_sheet("1qbsaLHmMx6znIPDQ26psIfyXqo7tkF_ifANUIk0wKlw")
...
layout_column_wrap(
  ...
  card(
    ...
    card_body(
      ...
      count() %>% 
      ungroup() %>% 
      hchart(type="pie",hcaes(x=demo1,y=n),name="Nivel Jerárquico") %>% 
  ...
  card(
    ...
    card_body(
     principal %>%
      filter(estado=="Completado") %>% 
      group_by(floor_date(fecha, "day"),demo1) %>%
      count() %>% 
      rename("dias"=1) %>% 
      ungroup() %>% 
      mutate(tiempo = dias %>% datetime_to_timestamp()) %>% 
      hchart("spline", hcaes(x = tiempo, y = n, group = demo1)) %>%
      hc_title(text = "Respuestas por Nivel Jerárquico") %>%
      hc_subtitle(text = "Tendencia diaria") %>% 
      hc_xAxis(type = "datetime", labels = list(format = "{value:%d-%b}"), title = list(text = "")) %>% 
      hc_yAxis(title = list(text = "Cantidad de Respuestas")) %>%
...
layout_column_wrap(
  ...
  card_body(
   contador_pais %>% 
    ...
    hc_add_series(type = "bar",data = contador_pais,hcaes(x = pais, y = prop_Completado, z = respuestas_Completado, moe = moe),
...
contador_area <- principal %>% 
  ...
  mutate(label_area2 = paste0(pais," - ",area2)) %>%
  group_by(label_area2,estado) %>% 
  summarise(respuestas = n()) %>% 
  mutate(prop = round(respuestas/sum(respuestas)*100,0)) %>%
  pivot_wider(names_from = estado, values_from = c(respuestas,prop), values_fill = 0) %>% 
  mutate(total_respuestas = rowSums(across(starts_with("respuestas"))),
...
layout_column_wrap(
  ...
  card_body(
   contador_area %>% 
    hchart("bar", hcaes(x = label_area2, y = total_prop, z = total_respuestas), name = "Población", showInLegend = TRUE,
           borderRadius = 10,
           dataLabels=list(enabled=TRUE, format="{point.z:.0f}")) %>%
    hc_add_series(type = "bar",data = contador_area,hcaes(x = label_area2, y = prop_Completado, z = respuestas_Completado),
                  name = "Completado", color = "#29BEB4",showInLegend = TRUE,
                  borderRadius = 10,
                  dataLabels=list(enabled=TRUE, format="<span style='color:{series.color}'>{point.z:.0f} ({point.y:.0f}%)</span>")) %>%
...
contador_area3 <- principal %>% 
  ...
  mutate(label_area3 = paste0(pais," - ",area3)) %>%
  group_by(label_area3,estado) %>% 
  summarise(respuestas = n()) %>% 
  mutate(prop = round(respuestas/sum(respuestas)*100,0)) %>%
  pivot_wider(names_from = estado, values_from = c(respuestas,prop), values_fill = 0) %>% 
  mutate(total_respuestas = rowSums(across(starts_with("respuestas"))),
...
layout_column_wrap(
  ...
  card_body(
   contador_area3 %>% 
    hchart("bar", hcaes(x = label_area3, y = total_prop, z = total_respuestas), name = "Población", showInLegend = TRUE,
           borderRadius = 10,
           dataLabels=list(enabled=TRUE, format="{point.z:.0f}")) %>%
    hc_add_series(type = "bar",data = contador_area3,hcaes(x = label_area3, y = prop_Completado, z = respuestas_Completado),
                  name = "Completado", color = "#29BEB4",showInLegend = TRUE,
                  borderRadius = 10,
                  dataLabels=list(enabled=TRUE, format="<span style='color:{series.color}'>{point.z:.0f} ({point.y:.0f}%)</span>")) %>%
    hc_title(text = "Avance de Respuestas por Sub área") %>%
    # hc_subtitle(text = "Frecuencias por Nivel de Respuesta") %>%
    hc_xAxis(title = list(text = "")) %>% 
    hc_yAxis(title = list(text = "Porcentaje"),ceiling = 100, labels = list(format = "{text}%")) %>%
    hc_colors(c("#EBEBEB"))%>%
...
layout_column_wrap(
  ...
  card_body(
   contador_lider1 %>% 
    hchart("bar", hcaes(x = lider1, y = total_prop, z = total_respuestas), name = "Población", showInLegend = TRUE,
           borderRadius = 10,
           dataLabels=list(enabled=TRUE, format="{point.z:.0f}")) %>%
    hc_add_series(type = "bar",data = contador_lider1,hcaes(x = lider1, y = prop_Completado, z = respuestas_Completado),
                  name = "Completado", color = "#29BEB4",showInLegend = TRUE,
                  borderRadius = 10,
                  dataLabels=list(enabled=TRUE, format="<span style='color:{series.color}'>{point.z:.0f} ({point.y:.0f}%)</span>")) %>%
    hc_title(text = "Avance de Respuestas por Líderes") %>%
    # hc_subtitle(text = "Frecuencias por Nivel de Respuesta") %>%
    hc_xAxis(title = list(text = "")) %>% 
    hc_yAxis(title = list(text = "Porcentaje"),ceiling = 100, labels = list(format = "{text}%")) %>%
    hc_colors(c("#EBEBEB"))%>%
